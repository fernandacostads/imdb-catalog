...
public class DirectorController {
    ...
    public DirectorController(DirectorService directorService, Scanner scanner, MovieController movieController) {
    ...
    private void editDirectorDetails(Movie selectedMovie) {
        ...
        List<Director> directors = selectedMovie.getDirectors();
        for (int i = 0; i < directors.size(); i++) {
            System.out.println((i + 1) + " - " + directors.get(i).getName() + " (ID: " + directors.get(i).getId() + ")");
        }
        ...
        if (directorService.findDirectorByName(newName) != null && directorService.findDirectorByNationality(newNationality) != null) {
            System.out.println("Director with the same name and nationality already exists.");
            return;
        }
    ...
    private void removeDirector(Movie selectedMovie) {
        if (selectedMovie.getDirectors().size() == 1) {
            System.out.println("You cannot remove the only director of the movie. The movie must have at least one director.");
            return;
        }
